<?xml version="1.0" encoding="UTF-8"?>
<tileset name="deco" tilewidth="64" tileheight="64" spacing="64" margin="64">
 <image source="deco.png" width="448" height="320"/>
 <tile id="0">
  <properties>
   <property name="Colour" value="white"/>
   <property name="Direction" value="0.0"/>
   <property name="Distance" value="3.0"/>
   <property name="Invulnerable" value="true"/>
   <property name="Light" value="true"/>
   <property name="LightType" value="cone"/>
   <property name="Name" value="Lamp2"/>
   <property name="Radius" value="60.0"/>
   <property name="Type" value="Deko"/>
  </properties>
 </tile>
 <tile id="1">
  <properties>
   <property name="Colour" value="white"/>
   <property name="Direction" value="0.0"/>
   <property name="Distance" value="3.0"/>
   <property name="Invulnerable" value="true"/>
   <property name="Light" value="true"/>
   <property name="LightType" value="cone"/>
   <property name="Name" value="Lamp3"/>
   <property name="Radius" value="60.0"/>
   <property name="Type" value="Deko"/>
  </properties>
 </tile>
 <tile id="2">
  <properties>
   <property name="Colour" value="white"/>
   <property name="Direction" value="0.0"/>
   <property name="Distance" value="3.0"/>
   <property name="Invulnerable" value="true"/>
   <property name="Light" value="true"/>
   <property name="LightType" value="cone"/>
   <property name="Name" value="Lamp4"/>
   <property name="Radius" value="60.0"/>
   <property name="Type" value="Deko"/>
  </properties>
 </tile>
 <tile id="3">
  <properties>
   <property name="Invulnerable" value="true"/>
   <property name="Name" value="Sign1"/>
   <property name="Type" value="Deko"/>
  </properties>
 </tile>
 <tile id="4">
  <properties>
   <property name="Invulnerable" value="true"/>
   <property name="Name" value="Sign2"/>
   <property name="Type" value="Deko"/>
  </properties>
 </tile>
 <tile id="5">
  <properties>
   <property name="Colour" value="yellow"/>
   <property name="Direction" value="180.0"/>
   <property name="Distance" value="5.0"/>
   <property name="Invulnerable" value="true"/>
   <property name="Light" value="true"/>
   <property name="LightType" value="cone"/>
   <property name="Name" value="Lamp1"/>
   <property name="Radius" value="90.0"/>
   <property name="Type" value="Deko"/>
  </properties>
 </tile>
</tileset>
